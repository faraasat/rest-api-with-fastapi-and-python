...
async def test_create_comment(async_client: AsyncClient, created_post: dict):
    ...
    assert response.status_code == 201
    assert {"id": 0, "body": body,
            "post_id": created_post["id"]}.items() <= response.json().items()
...
@pytest.mark.anyio
async def test_get_missing_post_with_comment(async_client: AsyncClient, created_post: dict, created_comment: dict):

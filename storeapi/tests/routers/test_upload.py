...
def sample_image(fs) -> pathlib.Path:
    path = (pathlib.Path(__file__).parent / "assets" / "myfile.jpg").resolve()
    fs.create_file(path)
    return path
...
@pytest.fixture(autoouse=True)
def aiofile_mock_open(mocker, fs):
    mock_open = mocker.patch("aiofiles.open")

    @contextlib.asynccontextmanager
    async def async_file_open(fname: str, mode: str = "r"):
        out_fs_mock = mocker.AsyncMock(name=f"asnyc_file_open:{fname!r}/{mode!r}")
